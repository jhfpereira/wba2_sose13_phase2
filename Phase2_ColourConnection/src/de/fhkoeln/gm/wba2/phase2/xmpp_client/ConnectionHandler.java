...
public class ConnectionHandler {
		...
		public boolean publishWithPayload(String node_id, String payload_data) {
			...
			if(node != null) {
				...
				SimplePayload payload = new SimplePayload("palette", "", payload_data);
		...
		public boolean deleteNode(String node_id) {
			...
			try {
				...
				System.err.println("Couldn't delete the node with the ID \"" + node_id + "\"");
				return false;
			}
		...
		private void attachListenerToSubscribedNodes() {
			...
			for(Affiliation curr: affiliations) {
				if(curr.getType() != Affiliation.Type.publisher) {
					try {
						pubsub_man.getNode(curr.getNodeId()).addItemEventListener(listener);
					} catch (XMPPException e) {
						System.err.println("Couldn't get Node for attaching Listener");
					}
				}
			}
		}
